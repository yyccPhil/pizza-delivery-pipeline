CREATE VIEW orders_by_pizza_types AS

SELECT state, type, total_number_of_pizzas_sold, gross_sales, (gross_sales / population) AS gross_sales_per_capita, number_of_unique_customers
FROM (
  SELECT state, type, sum(qty) AS total_number_of_pizzas_sold, sum(retail_price) AS gross_sales, count(DISTINCT customer_id) AS number_of_unique_customers
  FROM orders o
...
SELECT state, type, gross_sales_per_capita
FROM orders_by_pizza_types
WHERE state = {}
ORDER BY total_number_of_pizzas_sold DESC
LIMIT 3
